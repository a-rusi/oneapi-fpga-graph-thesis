...
// SystemVerilog created from k0_ZTSZZ12parallel_bfsRSt6vectorIS_IiSaI0000E_function_cra_agent
...
module k0_ZTSZZ12parallel_bfsRSt6vectorIS_IiSaI0000E_function_cra_agent (
    ...
    output wire [1055:0] kernel_arguments,
    ...
    wire [1055:0] kernel_arg_bit_join_q;
    ...
    reg [1055:0] pipe_reg0_kernel_arguments_q;
    reg [0:0] pipe_reg0_start_q;
    reg [1055:0] pipe_reg1_kernel_arguments_q;
    ...
    wire [4:0] dupName_30_address_ref_x_q;
    ...
    // printf_bit(BITSELECT,205)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,206)
    ...
    // readdata_valid_stage1_reg(REG,213)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // finish_counter_NO_SHIFT_REG(REG,105)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // printf_reset_bit(BITSELECT,208)
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,100)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // readdata_valid_stage2_reg(REG,214)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_11_address_ref_x(CONSTANT,324)
    ...
    // global_offset_reg_upper_0(REG,121)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,115)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_0_bit_join(BITJOIN,112)
    ...
    // global_offset_reg_upper_1(REG,123)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,117)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_1_bit_join(BITJOIN,113)
    ...
    // global_offset_reg_upper_2(REG,125)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,119)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_2_bit_join(BITJOIN,114)
    ...
    // global_size_reg_upper_0(REG,136)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,130)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_0_bit_join(BITJOIN,127)
    ...
    // global_size_reg_lower_1(REG,132)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_1_bit_join(BITJOIN,128)
    ...
    // global_size_reg_lower_2(REG,134)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_2_bit_join(BITJOIN,129)
    ...
    // dupName_23_address_ref_x(CONSTANT,404)
    ...
    // kernel_arg_bit_join(BITJOIN,155)
    ...
    // pipe_reg0_kernel_arguments(REG,201)
    always @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg1_kernel_arguments(REG,203)
    always @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // local_size_reg_2_buffered(REG,162)
    always @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // not_started(LOGICAL,172)
    assign not_started_q = ~ (started_NO_SHIFT_REG_q);

    // start_is_or_going_high(LOGICAL,230)
