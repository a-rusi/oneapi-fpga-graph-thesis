...
#pragma omp parallel for reduction(+ \
                                   : awake_count) schedule(dynamic, 1024)
...
pvector<NodeID> NAIVEBFS(const Graph &g, NodeID source)
{
  pvector<NodeID> parent = InitParent(g);
...
{
  CLApp cli(argc, argv, "naive breadth-first search");
  if (!cli.ParseArgs())
    return -1;
  Builder b(cli);
  Graph g = b.MakeGraph();
  SourcePicker<Graph> sp(g, cli.start_vertex());
  auto BFSBound = [&sp](const Graph &g) { return NAIVEBFS(g, sp.PickNext()); };
